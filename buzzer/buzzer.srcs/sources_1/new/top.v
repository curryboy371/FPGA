...
module top(clk, reset, btn, motor_direction, PWM_OUT, in1_in2, led, an, seg, buzzer, servo);
    input clk, reset;
    input [4:0] btn;
    ...
    wire w_tick_1s;
    wire w_toggle_1s; 
    tick_generator_1s #(.TICK_COUNT(100_000_000)) u_tick_generator_1s ( // 1s @100MHz  1s : 100_000_000
    .clk(clk),
    .reset(reset),
    .tick_1s(w_tick_1s),
    ...
    always @(posedge clk or posedge reset) begin
        if (reset) begin
            ...
            if (oven_state == RUNNING && oven_time <= 0 && w_tick_1s) begin
