...
module ahu (
                ...
                RUNNING: begin
                    ...
                    if(device_enable[INDEX_DHT11]) begin
                        if(data_valid) begin
                            ...
                            // 온도차 계산
                            if (temp_int > ahu_setting_values[INDEX_TEMP]) begin
                                if (temp_int - ahu_setting_values[INDEX_TEMP] >= 10)
                                    r_duty_cycle <= 10;
                                else
                                    r_duty_cycle <= temp_int - ahu_setting_values[INDEX_TEMP];
                            end else begin
                                r_duty_cycle <= 0;
                            end
    ...
    always @(posedge clk or posedge reset) begin
        if (reset) begin
            ...
            if (ahu_state == RUNNING && ahu_getting_values[INDEX_TIME] <= 0 && tick_1s) begin
                buzzer_pulse <= ALARM; // 종료 알림
            end
        end
    end

    buzzer_sequence u_buzzer_sequence1 (
        .clk(clk),
        .reset(reset),
        .tick(tick),
        .toggle_1s(toggle_1s),
    ...
    pwm_duty_cycle_control u_pwm_duty_cycle_control1 (
        .clk(clk),
    ...
    wire [7:0] w_rx_data;
    wire w_rx_done;
    uart_controller u_uart_controller1(
        .clk(clk),
        .reset(reset),
